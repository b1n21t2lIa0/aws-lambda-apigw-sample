...
FIELD_MAP = {
    "hostname": ["hostname"],
    "count": ["count", "Count"],
    "bgp_peer_down": ["BGP_Peer_Down_TG"],
}
...
def build_events_block(parsed_messages: List[Dict[str, Any]]) -> str:
    ...
    例如：
    • Events (2):
      1) Host `10.250.253.1`  Count `2`  BGP `bgp_peer_down 1.2.3.4 on bundle-ether...`
      2) Host `10.250.253.2`  Count `2`  BGP `...`
    """
    ...
    for idx, msg in enumerate(parsed_messages, start=1):
        ...
        bgp_peer_down  = pick_field(extracted, FIELD_MAP["bgp_peer_down"])

        piece = f"{idx}) Host `{host}`  Count `{cnt}`  BGP `{bgp_peer_down}`"
        lines.append(piece)

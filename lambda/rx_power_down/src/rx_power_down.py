...
def build_events_block(parsed_messages: List[Dict[str, Any]]) -> str:
    ...
    例如：
    • Events (2):
      1) Host `10.250.253.1`  Count `2`  RX `rx_power_low 1.2.3.4 on bundle-ether...`
      2) Host `10.250.253.2`  Count `2`  RX `...`
    """
    ...
    for idx, msg in enumerate(parsed_messages, start=1):
        ...
        # rx_power_low  = pick_field(extracted, FIELD_MAP["rx_power_low"])

        # piece = f"{idx}) Host `{host}`  Count `{cnt}`  RX `{rx_power_low}`"
        piece = f"{idx}) Host `{host}`  Count `{cnt}`"
        lines.append(piece)
